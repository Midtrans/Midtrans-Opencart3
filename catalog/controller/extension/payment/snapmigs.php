...
class ControllerExtensionPaymentSnapmigs extends Controller {
  ...
  public function index() {
    ...
    $data['client_key'] = $this->config->get('payment_snapmigs_client_key');
    $data['environment'] = $this->config->get('payment_snapmigs_environment');
  ...
  public function process_order() {
    ...
    if ($this->config->get('config_currency') != 'IDR') {
      ...
      else if ($this->config->get('payment_snapmigs_currency_conversion') > 0) {
        foreach ($item_details as &$item) {
          $item['price'] = intval($item['price']
              * $this->config->get('payment_snapmigs_currency_conversion'));
        }
        unset($item);
    ...
    $serverKey = $this->config->get('payment_snapmigs_server_key');
    Veritrans_Config::$serverKey = $serverKey;

    Veritrans_Config::$isProduction =
        $this->config->get('payment_snapmigs_environment') == 'production'
        ? true : false;
    ...
    if($this->config->get('payment_snapmigs_oneclick') == 1){

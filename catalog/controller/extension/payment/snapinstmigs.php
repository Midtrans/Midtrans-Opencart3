...
class ControllerExtensionPaymentSnapinstmigs extends Controller {
  ...
  public function index() {
    ...
    $data['client_key'] = $this->config->get('payment_snapinstmigs_client_key');
    $data['min_txn'] = $this->config->get('payment_snapinstmigs_min_txn');
    $data['environment'] = $this->config->get('payment_snapinstmigs_environment');
  ...
  public function process_order() {
    ...
    if ($this->config->get('config_currency') != 'IDR') {
      ...
      else if ($this->config->get('payment_snapinstmigs_currency_conversion') > 0) {
        foreach ($item_details as &$item) {
          $item['price'] = intval($item['price']
              * $this->config->get('payment_snapinstmigs_currency_conversion'));
        }
        unset($item);
    ...
    Veritrans_Config::$serverKey = $this->config->get('payment_snapinstmigs_server_key');

    error_log(Veritrans_Config::$serverKey);    

    Veritrans_Config::$isProduction =
        $this->config->get('payment_snapinstmigs_environment') == 'production'
        ? true : false;

    error_log($this->config->get('payment_snapinstmigs_environment'));        
    ...
    if ($transaction_details['gross_amount'] >= $this->config->get('payment_snapinstmigs_min_txn')){
